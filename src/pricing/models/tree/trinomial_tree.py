...
class TrinomialTree(Model):
    ...
    def price(
        ...
        if build_tree or not hasattr(self, "root") or self.root is None:
            self.option = option
            self._compute_parameters(self.market.stock_price, dividend=False)
    ...
    def _build_tree(self, compute_greeks: bool = False) -> Node:
        ...
        root = Node(S=self.market.stock_price, proba=1.0)
        ...
        for i in range(self.N):
            dividend = (div_step == i + 1 and self.market.div > 0)
    ...
    def gamma(self) -> float:
        ...
        h, S0 = self._get_bump(), self.market.stock_price
    ...
    def vanna(self, option: Option, bump_sigma: float = 0.01, bump_S: float = 0.01) -> float:
        ...
        # V(S+, sigma+)
        self.market.stock_price, self.market.sigma = S0 * (1 + bump_S), sigma0 * (1 + bump_sigma)
    ...
    def _compute_parameters(
        ...
        # inputes probas
        var = compute_variance(S, self.market.int_rate, self.delta_t, self.market.sigma)
        fwd = compute_forward(S, self.market.int_rate, self.delta_t)
        E = fwd - dividend
    ...
    def _compute_div_step(self) -> Optional[int]:
        ...
        if not getattr(self.market, "div_date", None) or not self.market.div:
            return None
        years = (self.market.div_date - self.pricing_date).days / 365
