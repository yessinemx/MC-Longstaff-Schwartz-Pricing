...
from ...others.funcs import iter_column, compute_probabilities, compute_forward
...
class Node:
    ...
    def _handle_dividend_case(self, tree: Optional["TrinomialTree"], esp_target: float, p_mid: float) -> None:
        ...
        fwd_from_snapped = self.next_mid.S * np.exp(tree.market.int_rate * tree.delta_t)
        var_from_snapped = (self.next_mid.S**2 * np.exp(2 * tree.market.int_rate * tree.delta_t) 
    ...
    def _esperance(self, tree: Optional["TrinomialTree"], is_dividend_step: bool = False) -> float:
        ...
        fwd = compute_forward(self.S, tree.market.int_rate, tree.delta_t)
        return fwd - tree.market.div if is_dividend_step else fwd
    ...
    def _discount(self) -> float:
        ...
        return np.exp(-self.tree.market.int_rate * self.tree.delta_t)
